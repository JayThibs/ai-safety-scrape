...
class AlignmentNewsletter:
    def __init__(
        self,
        input_folder_path="data/raw/alignment_newsletter/",
    ...
    def fetch_entries(self):
        ...
        with concurrent.futures.ProcessPoolExecutor(self.n_threads) as executor:
            executor.map(self.fetch_individual_entries, self.df.itterrows())
        return self.alignment_newsletter

    def fetch_individual_entries(self, entry_index):
        for index, row in self.df.iterrows():
            print(f"Processing entry {index}/{len(self.df)}")
            if row["Summary"] != None:
                try:
                    paper_url = str(
                        self.ws.cell(row=index + 1, column=3).hyperlink.target
                    )
                    newsletter_url = str(
                        self.ws.cell(row=index + 1, column=8).hyperlink.target
                    )
                except:
                    paper_url = ""
                    newsletter_url = ""
                if "gradientscience" in paper_url:
                    ...
                    try:
                        arxiv_id = re.findall(
                            r"(?:arXiv:|abs/)(\d{4}\.\d{4,5})", paper_url
                        )[0]
                    except:
                        arxiv_id = None
                        pass
                ...
                self.alignment_newsletter[str(entry_index)] = {
