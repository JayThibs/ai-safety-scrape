...
from .grobid_tei_paper import Paper

from .grobid_util import parse_bib_entry, extract_paper_metadata_from_grobid_xml
from .citation_util import SINGLE_BRACKET_REGEX, BRACKET_REGEX, BRACKET_STYLE_THRESHOLD
from .citation_util import is_expansion_string, _clean_empty_and_duplicate_authors_from_grobid_parse
from .refspan_util import sub_spans_and_update_indices
...
def convert_body_head_to_h(body: BeautifulSoup) -> BeautifulSoup:
    """
    Convert body head to h
    :param body:
    :return:
    """
    for h in body.find_all('head'):
        if 'n' not in h.attrs:
            pass
        elif len(h.attrs['n'].split('.')) == 1:
            h.name = 'h1'
        elif len(h.attrs['n'].split('.')) == 2:
            h.name = 'h2'
        elif len(h.attrs['n'].split('.')) == 3:
            h.name = 'h3'
        else:
            h.name = 'h4'
    return body
...
def remake_ref_tags(body: BeautifulSoup) -> BeautifulSoup:
    """
    Remake ref tags
    :param body:
    :return:
    ...
    for ref in body.find_all('ref'):
        # remove all [] from ref text
        ref_text = ref.text.replace('[', '').replace(']', '')
        if 'target' in ref.attrs:
            ref_text = '['+ref_text+']' + '['+ref.attrs['target']+']'
        else:
            ref_text = '['+ref_text+']'
        ref.replace_with(ref_text)
    return body


def extract_body_as_markdown_from_tei(sp: BeautifulSoup) -> str:
    if not sp.body:
        return ''
    body = sp.body
    body = convert_body_head_to_h(body)
    body = remake_ref_tags(body)
...
def extract_body_text_from_tei_xml(
    ...
    body_text = []
    if sp.body:
        sp.body = convert_body_head_to_h(sp.body)
        # sp.body = remake_ref_tags(sp.body)
...
def convert_tei_xml_soup_to_s2orc_json(soup: BeautifulSoup, paper_id: str, pdf_hash: str) -> Paper:
    ...
    body_markdown = extract_body_as_markdown_from_tei(soup)
...
def convert_tei_xml_file_to_s2orc_json(tei_file: str, pdf_hash: str = "") -> Paper:
    ...
    paper_id = str(tei_file).split('/')[-1].split('.')[0]
